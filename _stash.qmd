...
Creating unit tests usually involves three steps: 1) add any data or values you will need to run the test to the top of the test “scope” or “execution environment”, 2) create the `object`, which is the output or behavior created by the test function 3) define the `expected` value, object, or behavior of the test function. 
...
> [**Mezzo-iteration:**](https://r-pkgs.org/testing-basics.html#run-tests) As one file’s-worth of functions and their associated tests start to shape up, you will want to execute the entire file of associated tests, perhaps with `testthat::test_file()`:
...
── Error (test-get_column_class.R:52:5): get_column_class() works ─────
...
ℹ Testing utap
✔ | F W S  OK | Context
✔ |         1 | pull_binary_cols [0.2s]      
✔ |         1 | pull_cat_cols [0.1s]         
✔ |        25 | check_binary_vec [0.2s]       
✔ |        16 | check_facet_vec [0.1s]        
✔ |         1 | pull_facet_cols              
✔ |        16 | get_column_class [0.2s]          
...
-   The test data (`.rds`) and code (`make_<data>.R`) are stored in `tests/testthat/fixtures/`:
    ...
    tests/testthat/fixtures/
      ...
      ├── col_types_test.rds
      ├── facet_vec_test.rds
      ├── make_pull_cols_test.R
      ├── make_binary_vec_test.R
      ├── make_col_types_test.R
      └── make_facet_vec_test.R
