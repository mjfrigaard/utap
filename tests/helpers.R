binary_checks_helper <- function(type) {
  switch(type,
    # logical binary (with missing)
    log_na_bin = sample(x = c(TRUE, FALSE, NA, TRUE), 5, TRUE),
    # logical binary (no missing)
    log_bin = sample(x = c(TRUE, FALSE, FALSE, TRUE), 5, TRUE),
    # integer binary (with missing)
    int_bin = sample(x = c(0L, 1L, NA_integer_, 1L), 5, TRUE),
    # two-level character facet variable (with missing)
    chr_3l_na_facet = sample(
      x = c("A", "B", NA_character_, "B"),
      5, TRUE
    ),
    ...
    chr_4l_facet = sample(x = c("A", "B", "C", "D"), 5, TRUE),
    # three level integer variable (with missing)
    int_3l_na = sample(x = c(0L, 1L, NA_integer_, 2L), 5, TRUE),
    # three level double variable (with missing)
    dbl_3l_na = sample(x = c(0.5, 1.2, NA_real_, 0.001), 5, TRUE),
    ...
    ord_3l = factor(sample(x = c("high", "low", "med"), 5, TRUE),
      levels = c("low", "high", "med"),
      ordered = TRUE
    ),
    ...
    fct_10l = factor(
      x = sample(c(LETTERS[1:10]), 10, TRUE),
