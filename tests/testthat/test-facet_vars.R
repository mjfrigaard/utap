test_that("facet_vars() works", {
  ...
  chr_vars_test <- tibble::tibble(
        ...
        chr_05 = sample(x = c(LETTERS[1:5], NA_character_),
  ...
  ord_vars_test <- tibble::tibble(
      ...
      ord_05 = factor(sample(x = c("very low", "low", "medium",
                                   "high", "very high", NA_character_),
                               size = 100, replace = TRUE),
                          levels = c("very low", "low", "medium", "high", "very high"),
                          ordered = TRUE),
      # tests for six level ordered factor (with missing)
      ord_06 = factor(sample(x = c("very low", "low", "medium", "neutral",
                                   "high", "very high", NA_character_),
                               size = 100, replace = TRUE),
  ...
  fct_vars_test <- tibble::tibble(
        ...
        fct_06 = factor(x = sample(c("group a", "group b", "group c",
        ...
        fct_07 = factor(x = sample(c("group a", "group b", "group c",
